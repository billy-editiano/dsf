...
func HitCallback(hosts map[string]datasaur.Host) func() {
	return func() {
		response := make(chan HealthResponseWithName)
		wg := sync.WaitGroup{}

		// fetch all hosts concurrently
		for _, host := range hosts {
			wg.Add(1)
			go fetchHost(response, &wg, host)
		}

		go func() {
			// wait for all fetchHost to finish
			wg.Wait()
